...
The `@openflags/react` package builds on the core client and gives React applications an idiomatic hook-based API.

## Provider setup

```tsx
import { OpenFlagsProvider } from "@openflags/react"
...
```tsx
import { useFlag } from "@openflags/react"
