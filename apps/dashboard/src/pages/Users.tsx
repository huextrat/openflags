          ...
          <CardContent className="space-y-8 pt-2">
            {isPlatformAdmin && (
              <div className="rounded-2xl border border-white/5 bg-black/20 p-5 shadow-inner relative overflow-hidden">
                ...
                >
                  {error && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-400">
                      {error}
                    </motion.div>
                  )}
                  <div className="flex flex-col sm:flex-row gap-4 sm:items-end">
                    ...
                    <div className="w-full sm:w-[160px] space-y-2">
                      ...
                      <Select value={role} onValueChange={setRole}>
                        <SelectTrigger className="h-11 border-white/10 bg-white/[0.03] hover:bg-white/[0.05]" id="invite-role">
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <Button type="submit" className="h-11 px-8 sm:w-auto w-full shadow-[0_0_15px_rgba(139,92,246,0.2)]">
                      Send Invite
                    </Button>
                  </div>
                </Form.Root>
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                           ...
                           {u.id === currentUser?.id && (
                             <span className="inline-flex items-center rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-medium text-white/60">You</span>
                           )}
                           {isAdmin && (
                             <ShieldAlert className="h-3.5 w-3.5 text-amber-400/70" />
                           )}
                        </div>
                        <p className="text-xs text-white/40 font-mono mt-0.5 tracking-wider opacity-60">ID: {u.id.slice(0, 8)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 border-t border-white/5 sm:border-0 pt-3 sm:pt-0">
                      {isPlatformAdmin ? (
                            ...
                            <SelectContent className="border-white/10 bg-[#09090b]/90 backdrop-blur-xl">
                              {PLATFORM_ROLES.map((r) => (
                                <SelectItem key={r} value={r} className="capitalize text-xs">
                                  {r === "admin" ? <span className="text-amber-400">{r}</span> : r}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        ...
                        <span className={cn(
                          ...
                          isAdmin ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-white/5 text-white/60 border-white/10"
                        )}>{u.role}</span>
                      )}
                    </div>
                  </motion.li>
