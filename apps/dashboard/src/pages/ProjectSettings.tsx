    ...
    <div className="space-y-8 pb-12 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-white/50 text-sm">Manage configuration and dangerous actions for {project?.name || "this project"}.</p>
        </div>
      </div>
          ...
          <CardContent className="space-y-6 pt-4">
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-1 p-4 rounded-xl border border-white/5 bg-black/20">
                   <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Project Name</p>
                   <p className="text-lg font-medium text-white">{project?.name || "Loading..."}</p>
                </div>
                <div className="space-y-1 p-4 rounded-xl border border-white/5 bg-black/20">
                   <p className="text-xs font-semibold uppercase tracking-widest text-white/40">Project Slug</p>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    ...
                    <CardDescription className="text-white/50">Irreversible, destructive actions.</CardDescription>
                 </div>
              </div>
            </CardHeader>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-5 rounded-xl border border-red-500/10 bg-black/40">
                <div className="space-y-1 max-w-lg">
                  ...
                  <p className="text-sm text-white/50 leading-relaxed">
                    Permanently delete this project and all of its feature flags. This action is not reversible and will immediately break any apps depending on these flags.
                  </p>
                </div>
                <Button
                  variant="destructive"
          ...
          <DialogContent className="border-red-500/20 shadow-[0_0_50px_rgba(239,68,68,0.15)]">
            <Form.Root onSubmit={(e) => e.preventDefault()}>
              <DialogHeader>
                ...
                <DialogTitle className="text-center text-2xl text-red-400">Delete project</DialogTitle>
              </DialogHeader>
              
              <div className="space-y-6 mt-2">
                <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-center">
                  <p className="text-sm text-red-200/80 leading-relaxed">
                    This action will permanently delete <strong className="text-red-100 font-mono tracking-tight">{project?.name}</strong> and remove all associated data.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-black/30 p-3 rounded-xl border border-white/5 text-center mb-4">
                     <strong className="font-mono text-lg text-white/90 select-all">{project?.slug}</strong>
                  </div>
              ...
              <DialogFooter className="mt-8 border-t border-white/10 pt-6">
                <Button type="button" variant="ghost" className="hover:bg-white/5" onClick={() => setDeleteDialogOpen(false)}>
                  Cancel
                </Button>
                <Button
                  variant="destructive"
