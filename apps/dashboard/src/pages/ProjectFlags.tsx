              ...
              <DialogContent className="sm:max-w-md">
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-white/40 pt-1">Use a descriptive key, typically snake_case or kebab-case.</p>
                  </div>
                  <div className="space-y-4 rounded-xl border border-white/5 bg-white/[0.02] p-5">
                    <div className="flex items-center justify-between">
                      <div>
                         ...
                         <p className="text-[11px] text-white/40 mt-1">Is this flag active immediately on creation?</p>
                      </div>
                      <Switch
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-white/40 pt-1">
                      Comma or newline separated list of user IDs that will <strong>always</strong> evaluate to true.
                    </p>
                  </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Card className="text-center bg-transparent border-dashed border-white/20 relative overflow-hidden">
             ...
             <CardContent className="pt-16 pb-16 relative z-10 flex flex-col items-center">
              ...
              {canEditFlags && (
                <Button className="mt-8 gap-2 h-11 px-6 shadow-[0_0_20px_rgba(139,92,246,0.15)]" onClick={() => setDialogOpen(true)}>
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-1 items-start gap-4">
                     ...
                     <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-3">
                           <h3 className="font-mono text-[15px] font-semibold text-white tracking-tight truncate">{flag.key}</h3>
                           {!flag.enabled && (
                              <span className="inline-flex items-center rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-medium text-white/50 uppercase tracking-widest">Off</span>
                           )}
                        </div>
                        <div className="mt-2 flex items-center gap-4 text-xs text-white/40">
                           <div className="flex items-center gap-1.5" title={flag.users?.length ? flag.users.join(", ") : "No specific users targeted"}>
                             <Users className="h-3.5 w-3.5" />
                             <span>{flag.users?.length || 0} specifically targeted</span>
                           </div>
                           <div className="w-1 h-1 rounded-full bg-white/20" />
                           <span className="font-mono text-[10px] uppercase tracking-wider opacity-50">ID: {flag.id.slice(0,8)}</span>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 lg:gap-10 border-t border-white/5 pt-4 lg:pt-0 lg:border-t-0 pl-[3.25rem] lg:pl-0">
                     ...
                     <div className="flex flex-col gap-2 min-w-[5rem]">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-white/40">Status</span>
                        {canEditFlags ? (
                          <div className="flex items-center gap-3">
                            <Switch
                            ...
                            />
                            <span className={cn("text-xs font-medium", flag.enabled ? "text-violet-300" : "text-white/40")}>
                     ...
                     <div className="flex flex-col gap-2 w-full lg:w-48 xl:w-64 max-w-[200px]">
                        <div className="flex justify-between items-center text-[10px] font-semibold uppercase tracking-widest text-white/40">
                          <span>Traffic</span>
                          <span className="text-white/70 font-mono tracking-normal">{flag.rolloutPercentage}%</span>
                        </div>
                        {canEditFlags ? (
                          ...
                          <div className="h-2 w-full overflow-hidden rounded-full bg-white/10 mt-1">
                            <div className={cn("h-full rounded-full transition-all duration-500", flag.enabled ? "bg-violet-500 shadow-[0_0_10px_rgba(139,92,246,0.5)]" : "bg-white/20")} style={{ width: `${flag.rolloutPercentage}%` }} />
                          </div>
                        )}
                     </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
               <span>Edit Flag</span>
               <span className="font-mono text-sm px-2 py-0.5 rounded-md bg-white/10 text-violet-300">{editFlag?.key}</span>
            </DialogTitle>
          </DialogHeader>
          {editFlag && (
            <form className="space-y-6 mt-4" onSubmit={handleEditSave}>
              ...
              <div className="space-y-4 rounded-xl border border-white/5 bg-white/[0.02] p-5">
                 <div className="flex items-center justify-between">
                    <div>
                       ...
                       <p className="text-[11px] text-white/40 mt-1">Globally enable or disable this flag</p>
                    </div>
                    <Switch checked={editEnabled} onCheckedChange={setEditEnabled} className="data-[state=checked]:shadow-[0_0_15px_rgba(139,92,246,0.5)]" />
                 </div>
                 
                 <AnimatePresence>
                   {editEnabled && (
                     <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="space-y-3 pt-4 border-t border-white/5 overflow-hidden">
                       <div className="flex justify-between items-center">
                          <Label className="text-white">Percentage Rollout</Label>
                          <span className="inline-flex h-6 w-10 items-center justify-center rounded-md bg-white/10 text-xs font-semibold tabular-nums text-white">
                            {editRollout}%
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/50 transition-colors resize-y font-mono"
                />
                <p className="text-[11px] text-white/40 pt-1">Comma or newline separated list of user IDs that will always get this feature. Overrides rollout percentage.</p>
              </div>
              <DialogFooter className="border-t border-white/5 pt-6 mt-6">
