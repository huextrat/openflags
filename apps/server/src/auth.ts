...
export async function getSessionUser(db: Database, req: Request): Promise<UserRow | null> {
  ...
  const row = db
    .query(
      "SELECT u.id, u.email, u.password_hash, u.created_at FROM users u JOIN sessions s ON s.user_id = u.id WHERE s.id = ? AND s.expires_at > datetime('now')"
    )
...
export async function signup(
  ...
  const userId = nanoid()
  const passwordHash = await Bun.password.hash(password, { algorithm: "argon2id", memoryCost: 4, timeCost: 3 })
  try {
    db.run("INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)", [userId, email, passwordHash])
  } catch (err: unknown) {
  ...
  db.run("INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)", [sessionId, userId, expiresAt])
  return { body: { user: { id: userId, email } }, status: 201, sessionId }
}
...
export async function login(
  ...
  const user = db.query("SELECT id, email, password_hash, created_at FROM users WHERE email = ?").get(email) as
    | UserRow
    | undefined
  if (!user) {
...
export async function logout(db: Database, req: Request): Promise<{ body: unknown; status: number }> {
  const sessionId = getSessionIdFromRequest(req)
  if (sessionId) {
